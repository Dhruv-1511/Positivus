  ...
  return (
    <div className="flex flex-wrap justify-between items-center mt-10 px-28 lg:px-8">
      ...
      <nav className="flex flex-wrap items-center mt-4 gap-8 text-xl xl:gap-4 md:hidden">
        <Link to="/about" onClick={() => setMenu("/about")} className={`hover:underline rounded-lg ${isActive("/about")}`}>
          About us
        </Link>
        <Link to="/service" onClick={() => setMenu("/service")} className={`hover:underline rounded-lg ${isActive("/service")}`}>
          Services
        </Link>
        <Link to="/usecase" onClick={() => setMenu("/usecase")} className={`hover:underline rounded-lg ${isActive("/usecase")}`}>
          Use Cases
        </Link>
        <Link to="/price" onClick={() => setMenu("/price")} className={`hover:underline rounded-lg ${isActive("/price")}`}>
          Pricing
        </Link>
        <Link to="/blog" onClick={() => setMenu("/blog")} className={`hover:underline rounded-lg ${isActive("/blog")}`}>
          Blog
        </Link>
